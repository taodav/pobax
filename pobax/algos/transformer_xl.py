...
class PPOGTrXL:
    ...
    def loss(self, params, traj_batch, memories_batch, gae, targets):
        ...
        memories_mask = jnp.concatenate(
            (memories_mask, jnp.zeros(memories_mask.shape[:-1] + (args.window_grad - 1,), dtype=jnp.bool_)), axis=-1)
        ...
        traj_batch, targets, gae = jax.tree_util.tree_map(
            lambda x: jnp.reshape(x, (-1, args.window_grad) + x.shape[2:]), (traj_batch, targets, gae))

        # NETWORK OUTPUT
        pi, value = self.network.apply(params, memories_batch, obs, memories_mask,
...
def env_step(runner_state, unused, env, network, env_params, args: TransformerHyperparams):
    ...
    memories_mask = jnp.where(last_done[:, None, None, None],
                              jnp.zeros((args.num_envs, args.num_heads, 1, args.window_mem + 1), dtype=jnp.bool_),
                              memories_mask)
    ...
    memory_indices = jnp.arange(0, args.window_mem)[None, :] + step_env_currentloop * jnp.ones((args.num_envs, 1),
                                                                                               dtype=jnp.int32)
    ...
    runner_state = (
    train_state, env_state, memories, memories_mask, memories_mask_idx, obsv, done, step_env_currentloop + 1, rng)
...
def make_train(args: TransformerHyperparams, rand_key: jax.random.PRNGKey):
    ...
    def train(sweep_args_dict, rng):
        ...
        agent = PPOGTrXL(network, double_critic=double_critic, ld_weight=ld_weight, vf_coeff=vf_coeff,
        ...
        def _update_step(runner_state, i):
            ...
            _, last_val, _ = network.apply(train_state.params, memories, last_obs, memories_mask,
                                           method=network.model_forward_eval)
            ...
            if args.debug:
                ...
                def callback(info):
                    ...
                    if args.show_discounted:
                        ...
                        avg_return_values = jnp.mean(
                            info["returned_discounted_episode_returns"][info["returned_episode"]])
                    else:
