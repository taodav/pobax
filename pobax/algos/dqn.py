...
def make_train(config):
    ...
    config["UPDATES_PER_EPOCH"] = int(config["NUM_UPDATES"] // config["NUM_EPOCHS"])
    ...
    def train(rng):
        ...
        def _epoch_step(runner_state, unused):
            ...
            def _update_step(runner_state, unused):
                ...
                if config.get("DEBUG"):
                    def callback(metrics):
                        if metrics["timesteps"] % 100 == 0:
                            print(f"timestep {metrics['timesteps']}, returns: {metrics['returns']:.2f}, loss: {metrics['loss']:.2f}")
            ...
            runner_state, metrics = jax.lax.scan(
                _update_step, runner_state, None, config["UPDATES_PER_EPOCH"]
            )
        ...
        runner_state, (metrics, train_states) = jax.lax.scan(
            _epoch_step, runner_state, None, config["NUM_EPOCHS"]
        )

        return {"runner_state": runner_state, "metrics": metrics, 'train_states': train_states}
...
def main():
    ...
    outs.update({'config': config})

    # Save all results with Orbax
    orbax_checkpointer = orbax.checkpoint.PyTreeCheckpointer()
    save_args = orbax_utils.save_args_from_target(outs)

    results_path = Path(ROOT_DIR, 'results', f'{config["ENV_NAME"]}_{config["SEED"]}_{make_hash_md5(config)}')

    print(f"Saving results to {results_path}")
    orbax_checkpointer.save(results_path, outs, save_args=save_args)
